...
bool hasChanged(int16_t, int16_t *param);
...
static void nunchuk_print() {
	...
	static int16_t Jx, Jy, Ax, Ay, Bz, Bc;
...
#else
	bool change = false;
	if (hasChanged(nunchuk_joystickX(), &Jx))
	{
	...
	if (hasChanged(nunchuk_joystickY(), &Jy))
	{
	...
	if (hasChanged(nunchuk_accelY(), &Ax))
	{
		Serial.print("Accelerate Left/Right:");
		Serial.print(nunchuk_accelY(), DEC);
		Serial.print(",");
		change = true;
	}

	if (hasChanged(nunchuk_accelY(), &Ay))
	{
		Serial.print("Accelerate Forward/Back:");
		Serial.print(nunchuk_accelY(), DEC);
		Serial.print(",");
		change = true;
	}

	if (hasChanged(nunchuk_buttonZ(), &Bz))
	{
	...
	if (hasChanged(nunchuk_buttonC(), &Bc))
	{
	...
	if (change) Serial.print("\n");
#endif
}

#endif

bool hasChanged(int16_t newValue, int16_t * oldValue) {
